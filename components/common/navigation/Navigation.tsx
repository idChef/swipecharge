...
export const Navigation: FunctionComponent = ({}) => {
    ...
    return (
        <div className="fixed bottom-0 left-0 flex h-24 w-full items-center gap-6 border-t px-4  drop-shadow-sm">
            {ROUTES.map(({ path, icon, name }) => (
                <Link
                    key={name}
                    href={path}
                    className={`flex flex-col items-center justify-center text-gray-400 ${
                        router.pathname === path || router.pathname.startsWith(`${path}/`) ? "text-black" : ""
                    }`}
                >
